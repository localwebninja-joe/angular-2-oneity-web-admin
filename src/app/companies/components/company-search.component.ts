...
@Component({
  selector: 'bc-company-search',
  template: `
    <mat-card>
      <mat-card-title>Find a Company</mat-card-title>
      <mat-card-content>
        <mat-form-field>
          <input matInput placeholder="Search for a company" [value]="query" (keyup)="search.emit($event.target.value)">
        </mat-form-field>
